...
export default function SeatLayout({ seats = [], selected = [], onToggle, readOnly = false }) {
  ...
  return [...map.entries()]
    ...
    .sort(
      (a, b) =>
        categoryOrder[a[1][0]?.category] -
        categoryOrder[b[1][0]?.category]
    )
}, [seats])
            ...
            {rowSeats.map((seat) => (
              <button
                ...
                style={{
                  ...
                  cursor: seat.status !== "available" || readOnly ? "not-allowed" : "pointer",
                }}
              >
                {seat.number}
              </button>
